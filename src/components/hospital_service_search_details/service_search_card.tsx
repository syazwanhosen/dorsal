...
export const ServiceSearchCard = () => {
  ...
  const hospitalName = decodeURIComponent(params.get("name") || "");
  ...
  useEffect(() => {
    if (hospitalName) {
      getCategoriesByHospital(hospitalName)
  ...
  useEffect(() => {
    if (hospitalName && selectedServiceCategory) {
      getSubCategoriesByHospital(hospitalName, selectedServiceCategory)
        .then((data) => setSubcategories(data.sub_categories))
        .catch((error) =>
          console.error("Error fetching subcategories:", error)
        );
    }
  }, [hospitalName, selectedServiceCategory]);

  const fetchServices = () => {
    if (hospitalName && selectedServiceCategory && selectedSubcategory) {
      setLoading(true);
      setError("");

      getServices(hospitalName, selectedServiceCategory, selectedSubcategory)
        .then((data) => {
